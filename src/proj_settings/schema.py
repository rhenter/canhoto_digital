...
from drf_yasg.views import UI_RENDERERS, SPEC_RENDERERS, deferred_never_cache
...
def get_schema_view(
    ...
    class SchemaView(APIView):
        ...
        def get(self, request, version='', data_format=None):
        ...
        def with_ui(
            ...
            :return: a view instance
            """
            assert renderer in UI_RENDERERS, "supported default renderers are " + \
                                             ", ".join(UI_RENDERERS)
...
class CustomEndpointEnumerator(EndpointEnumerator):
    ...
    def should_include_endpoint(self, path, callback, app_name='', namespace='', url_name=None):
        if url_name in settings.DRF_YASG_EXCLUDE_VIEWS:
            return False
